...
1. ログインして web/themes/ に移動します。このフォルダー内に my_theme という名称のテーマディレクトリが作成します。
2. 次のコマンドでローカルリポジトリを作成します。
```
git clone https://github.com/DrupalMyTheme/MyTheme.git my_theme
```


# インストールとアップデート：テーマファイルを Drupal サイトにアップロードする場合
## 新規インストール
...
古いバージョンのテーマディレクトリ (my_theme など) をリネームするか、削除します。
